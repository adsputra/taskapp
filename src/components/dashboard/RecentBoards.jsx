  ...
  const renderBoard = (board, index, isShared = false) => (
    ...
    >
      <Link href={`/boards/${board.id}`}>
        <div className="group flex items-center gap-4 px-4 py-3.5 rounded-xl border border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:bg-white/70 dark:hover:bg-slate-800/70 transition-colors duration-200 cursor-pointer">
          {/* Icon */}
          <div
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <ArrowRight className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-blue-400 transition-colors" />
          </div>
        </div>
      </Link>
    </motion.div>
    ...
    <>
      ...
      {sharedBoards.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/60 dark:border-slate-800 shadow-sm dark:shadow-none overflow-hidden">
          ...
          <div className="px-3 py-2 divide-y divide-slate-50 dark:divide-slate-800">
